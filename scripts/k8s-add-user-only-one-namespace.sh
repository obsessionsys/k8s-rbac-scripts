...
if [[ ${userrole} = "developer" ]]; then
  getrole_exists=$(kubectl get role -n ${namespace} | grep developer)
  getrb_exists=$(kubectl get rolebinding -n ${namespace} | grep developer)
  if [[ "$getrole_exists" = "" ]] && [[ "$getrb_exists" = "" ]]; then
    _role_dev
    _create_developer
  else
    _patch_developer
  ...
  getrole_exists=$(kubectl get role -n ${namespace} | grep deployer)
